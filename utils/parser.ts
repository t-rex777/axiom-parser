...
export class AxiomParser {
	...
	static EXTRACT_URL_AND_MESSAGE_REGEX =
		/(?<url>(https?[^\s]+))\s\d+\s(?<message>(.+))/gm;
	...
	private getMessageElement(table: HTMLTableElement) {
		return table.querySelectorAll(AxiomParser.MESSAGE_SELECTOR)[2]
			.firstChild as HTMLDivElement;
	}

	private getUrlAndMessage(messageElement: HTMLDivElement) {
		const matches = messageElement.textContent?.matchAll(
			AxiomParser.EXTRACT_URL_AND_MESSAGE_REGEX,
		);
		if (matches) {
	...
	private storeMessage() {
		this.messageElement.innerHTML = prettyPrintJson.toHtml(
